...
pub struct FairLock<T> {
    waiting: Vec<AtomicBool>, // ロック獲得試行中のスレッド, n番目のスレッドがロック獲得中かどうか
    lock: AtomicBool,         // ロック用変数, スピンロック用
...
impl<T> FairLock<T> {
    ...
    pub fn lock(&self, idx: usize) -> FairLockGuard<T> {
        ...
        loop {
            ...
            // これ一番最初に一人だけロックを取りに行ったときは誰がこれを false にするんだろう
...
impl<'a, T> Drop for FairLockGuard<'a, T> {
    fn drop(&mut self) {
        ...
        if fl.waiting[next].load(Ordering::Relaxed) {
            ...
            // あれ、こっちはなんで lock を false にしなくていい？
            // lock 関数の 1 つ目の break; に引っかかるので loop を抜けられる
        } else {
